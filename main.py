...
def investir():
    ...
    try: valor=float(data.get('valor') or 0)
    except: return jsonify({'erro':'valor inválido'}),400
    if not nome: return jsonify({'erro':'nome obrigatório'}),400
    if valor<=0: return jsonify({'erro':'valor deve ser maior que zero'}),400
    with lock:
        if nome not in investidores:
            investidores[nome]={'saldo':float(CAPITAL_INICIAL),'total_investido':0.0}
        if valor>investidores[nome]['saldo']:
            return jsonify({'erro':'saldo insuficiente','saldo':investidores[nome]['saldo']}),400
